...
-- Relación: Medicamentos → Proveedores (CASCADA)
ALTER TABLE Medicamentos
ADD CONSTRAINT FK_Medicamentos_Proveedor
FOREIGN KEY (IdProveedor)
REFERENCES Proveedores(IdProveedor)
ON DELETE CASCADE;

-- Relación: Compras → Medicamentos (CASCADA)
ALTER TABLE Compras
ADD CONSTRAINT FK_Compras_Medicamento
FOREIGN KEY (IdMedicamento)
REFERENCES Medicamentos(IdMedicamento)
ON DELETE CASCADE;

-- Relación: Compras → Proveedores (SIN cascada)
ALTER TABLE Compras
...
-- 4️⃣ DetalleVenta → Medicamentos (CASCADA)
ALTER TABLE DetalleVenta
ADD CONSTRAINT FK_DetalleVenta_Medicamento
FOREIGN KEY (IdMedicamento)
REFERENCES Medicamentos(IdMedicamento)
ON DELETE CASCADE;
...
--trigger
CREATE OR ALTER TRIGGER trg_DeleteProveedor
ON Proveedores
INSTEAD OF DELETE
AS
BEGIN
    ...
    -- 1️⃣ Borrar Compras relacionadas con el proveedor
    DELETE FROM Compras
    WHERE IdProveedor IN (SELECT IdProveedor FROM deleted);

    -- 2️⃣ Borrar DetalleVenta relacionado con los medicamentos del proveedor
    DELETE FROM DetalleVenta
    WHERE IdMedicamento IN (
        SELECT IdMedicamento
        FROM Medicamentos
        WHERE IdProveedor IN (SELECT IdProveedor FROM deleted)
    );

    -- 3️⃣ Borrar Medicamentos del proveedor
    DELETE FROM Medicamentos
    WHERE IdProveedor IN (SELECT IdProveedor FROM deleted);

    -- 4️⃣ Borrar el proveedor
    DELETE FROM Proveedores
    WHERE IdProveedor IN (SELECT IdProveedor FROM deleted);

    -- 5️⃣ Ajustar IDENTITY de cada tabla afectada para que no queden "huecos"
    DECLARE @maxProveedor INT, @maxMedicamento INT, @maxCompra INT, @maxDetalle INT;

    SELECT @maxProveedor = ISNULL(MAX(IdProveedor), 0) FROM Proveedores;
    DBCC CHECKIDENT ('Proveedores', RESEED, @maxProveedor);

    SELECT @maxMedicamento = ISNULL(MAX(IdMedicamento), 0) FROM Medicamentos;
    DBCC CHECKIDENT ('Medicamentos', RESEED, @maxMedicamento);

    SELECT @maxCompra = ISNULL(MAX(IdCompra), 0) FROM Compras;
    DBCC CHECKIDENT ('Compras', RESEED, @maxCompra);

    SELECT @maxDetalle = ISNULL(MAX(IdDetalle), 0) FROM DetalleVenta;
    DBCC CHECKIDENT ('DetalleVenta', RESEED, @maxDetalle);

END;
GO

------- Reajustar IDs de Proveedores
DECLARE @maxProveedor INT;
SELECT @maxProveedor = ISNULL(MAX(IdProveedor),0) FROM Proveedores;
DBCC CHECKIDENT ('Proveedores', RESEED, @maxProveedor);

-- Reajustar IDs de Medicamentos
DECLARE @maxMedicamento INT;
SELECT @maxMedicamento = ISNULL(MAX(IdMedicamento),0) FROM Medicamentos;
DBCC CHECKIDENT ('Medicamentos', RESEED, @maxMedicamento);

-- Reajustar IDs de Compras
DECLARE @maxCompra INT;
SELECT @maxCompra = ISNULL(MAX(IdCompra),0) FROM Compras;
DBCC CHECKIDENT ('Compras', RESEED, @maxCompra);

-- Reajustar IDs de DetalleVenta
DECLARE @maxDetalle INT;
SELECT @maxDetalle = ISNULL(MAX(IdDetalle),0) FROM DetalleVenta;
DBCC CHECKIDENT ('DetalleVenta', RESEED, @maxDetalle);

-- Reajustar IDs de Clientes
DECLARE @maxCliente INT;
SELECT @maxCliente = ISNULL(MAX(IdCliente),0) FROM Clientes;
DBCC CHECKIDENT ('Clientes', RESEED, @maxCliente);

-- Reajustar IDs de Empleados
DECLARE @maxEmpleado INT;
SELECT @maxEmpleado = ISNULL(MAX(IdEmpleado),0) FROM Empleados;
DBCC CHECKIDENT ('Empleados', RESEED, @maxEmpleado);

-- Reajustar IDs de Ventas
DECLARE @maxVenta INT;
SELECT @maxVenta = ISNULL(MAX(IdVenta),0) FROM Ventas;
DBCC CHECKIDENT ('Ventas', RESEED, @maxVenta);

-- Reajustar IDs de HistorialVenta
DECLARE @maxHistorialVenta INT;
SELECT @maxHistorialVenta = ISNULL(MAX(IdHistorialVenta),0) FROM HistorialVenta;
DBCC CHECKIDENT ('HistorialVenta', RESEED, @maxHistorialVenta);

-- Reajustar IDs de HistorialCompras
DECLARE @maxHistorialCompra INT;
SELECT @maxHistorialCompra = ISNULL(MAX(IdHistorialCompra),0) FROM HistorialCompras;
DBCC CHECKIDENT ('HistorialCompras', RESEED, @maxHistorialCompra);

-- Reajustar IDs de HistorialUsuarios
DECLARE @maxHistorialUsuario INT;
SELECT @maxHistorialUsuario = ISNULL(MAX(IdHistorialUsuario),0) FROM HistorialUsuarios;
DBCC CHECKIDENT ('HistorialUsuarios', RESEED, @maxHistorialUsuario);
